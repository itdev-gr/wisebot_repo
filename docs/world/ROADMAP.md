...
| | Κατάσταση |
| --- | --- |
| Μοντέλο δεδομένων | `data/world/types.ts`, 363 γραμμές, παγωμένο — έχει ήδη Museum/Room/Exhibit/Riddle/Trail |
| Οθόνες | 10 αρχεία στο `components/world/` (World, CountryScreens, CityView, MuseumView, PlaceCard, RiddleGame, StampBook, PassportStamp) |
| Περιεχόμενο | **1 πόλη**: Αθήνα — 18 μέρη, 3 μουσεία, 4 διαδρομές, 3118 γραμμές |
| Συντεταγμένες | Αθήνα 18 (A:7 B:9 C:2, 6 με warnings) · Πόρτο 5 (όλα B, 3 με warnings) · κανένα D |
| Γλώσσες | 6 στο World (`el en de fr es it`), 2 στην υπόλοιπη εφαρμογή |
| Wiring | 4 additive γραμμές σε `App.tsx`, `constants.tsx`, `Layout.tsx`, `MakerPassport.tsx` + SEO entry — uncommitted |

Δηλαδή: ο σκελετός υπάρχει, το περιεχόμενο σχεδόν καθόλου. Αυτό ορίζει τη σειρά
παρακάτω.
...
παραγωγής σε *σειρά προτεραιότητας για μετά το pilot*. Παράγουμε τρεις πόλεις —
Αθήνα, Λισαβόνα, Πόρτο — και τις κάνουμε τέλειες.
