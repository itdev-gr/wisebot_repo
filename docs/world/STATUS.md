...
**Παρασκευή 18/9 βράδυ — πού έμεινε το engine session (C).** Main `5d73c4b`, CI πράσινο.
21 πόλεις (μπήκαν Λευκωσία, Ντουμπρόβνικ, Μαδρίτη, Βενετία), 60 overlays, 367 σελίδες World.
...
- **Νέος φύλακας «μπροστινή πόρτα».** Το `world.test.ts` ελέγχει τώρα τα `countries.<lang>.json`.
  Βρήκε 32 κάρτες πόλεων χωρίς μετάφραση· μεταφράστηκαν την ίδια νύχτα. Σήμερα λείπουν μόνο
  οι 8 κάρτες Μαδρίτης/Βενετίας (× de/es/fr/it), γραμμένες ονομαστικά στη λίστα
  `KNOWN_ABSENT_CARDS`. Τις γράφει το session γλωσσών· όποιος κάνει merge σβήνει τις γραμμές.
  Κάθε νέα πόλη που φτάνει μόνο σε el/en προσθέτει εκεί τις κάρτες της στο merge, φανερά.
