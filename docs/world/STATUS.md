...
**Παρασκευή 18/9 βράδυ — πού έμεινε το engine session (C).** Main `7fc9e09`, CI πράσινο.
19 πόλεις (μπήκαν Λευκωσία, Ντουμπρόβνικ), 60 overlays, 337 σελίδες World.
...
- **Νέος φύλακας «μπροστινή πόρτα».** Το `world.test.ts` ελέγχει τώρα τα `countries.<lang>.json`.
  Λείπουν 32 κάρτες πόλεων (Βερολίνο, Βουδαπέστη, Λισαβόνα, Πόρτο, Πράγα, Βιέννη, Λευκωσία,
  Ντουμπρόβνικ × de/es/fr/it), γραμμένες ονομαστικά στη λίστα `KNOWN_ABSENT_CARDS`. Τις γράφει
  το session γλωσσών· όποιος κάνει merge σβήνει τις γραμμές. Κάθε νέα πόλη που φτάνει μόνο σε
  el/en προσθέτει εκεί τις κάρτες της στο merge, φανερά.
