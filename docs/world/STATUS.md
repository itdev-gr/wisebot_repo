...
- **Νέος φύλακας «μπροστινή πόρτα».** Το `world.test.ts` ελέγχει τώρα τα `countries.<lang>.json`.
  ...
  #112, #124, #132, #135). Η λίστα `KNOWN_ABSENT_CARDS` είναι κενή: κάθε πόλη του main έχει
  κάρτα και στις έξι γλώσσες. Το Πόρτο είναι η μόνη πόλη χωρίς overlays (τα γαλλικά τρέχουν).
