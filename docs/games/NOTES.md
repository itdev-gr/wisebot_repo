...
6. **Κάθε νέο παιχνίδι = +1 στο `GAME_COUNT` στο `data/contentCounts.ts`, στο ΙΔΙΟ PR.** Το
   `data/contentCounts.test.ts` καρφώνει τη σταθερά στο πλήθος των καρτών του `GameCenter.tsx`. Το
   test μπήκε (5f81368) αφού το #144 είχε κάνει rebase, οπότε τα δύο ήταν πράσινα χωριστά και
   **κόκκινα μαζί** — main κόκκινο μέχρι το #153. Rebase στο main ΑΚΡΙΒΩΣ πριν το merge, όχι πριν
   το review.
7. **Το γραμματικό γένος των χαρακτήρων στα ελληνικά:** **η WiseBot, η Link** (θηλυκά)· **ο
   Pencilo, ο Crocus, ο Sparken** (αρσενικά). Το παραγόμενο περιεχόμενο τα είχε ανάποδα («ο
   WiseBot», «η Crocus», «ο Link») και ο ιδιοκτήτης το είδε στο Δωμάτιο Απόδρασης. Άρθρο, κτητικό
   («τα φτερά **της**») και επίθετο («χαρούμεν**η**») αλλάζουν μαζί — διορθώνεται με ανάγνωση,
   ποτέ με sed. Ελέγξτε το σε κάθε νέο ελληνικό κείμενο πριν το PR.
