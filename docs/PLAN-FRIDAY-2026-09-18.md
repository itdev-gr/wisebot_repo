...
(`646cc25`, μετά το PR #90). Ό,τι λέει «έγινε» είναι merged και live στο wisebot.gr. Ό,τι λέει «μένει»
...
| **Αφήγηση Kore** | Αθήνα, Θεσσαλονίκη, Παρίσι, Ρώμη, Λονδίνο (Άμστερνταμ: 2 κλιπ πιλότος) | Ηράκλειο, Ρόδος, Δελφοί (≈150 κλιπ, 50 η καθεμία), μετά Άμστερνταμ, Βαρκελώνη, Κων/πολη… (quota ≈100 κλιπ/μέρα, ο αγωγός είναι resumable) |
| **Μηχανή** | Χάρτης + Google/Apple Maps, σφραγίδα μόνο επί τόπου, «Είμαι εδώ!» ένα κουμπί (PR #88/#89), «Άλλη μία;», «Λάθος σημείο» (PinReport), τελετές seal, SEO 303 σελίδες, sync σφραγίδων → Supabase (push) | 7 ανοιχτά P0 tickets του `docs/world/TICKETS.md` (δες §4C· το `i18n-countries-24` είναι ήδη υλοποιημένο στα δεδομένα, θέλει μόνο κλείσιμο), GA4 events του World, `@vercel/analytics` |
...
Ανοιχτά PR στο GitHub: **κανένα** (εκτός από αυτό το έγγραφο). Τεστ: 720 πράσινα σε 25 αρχεία. Lint: 0 errors.
...
### A — Content: οι 5 πόλεις του Explorer που έμειναν, μετά Μαδρίτη και Βενετία
...
> Μαδρίτη και Βενετία (η σειρά του COUNTRIES.md: Ισπανία #3 Βαρκελώνη → Μαδρίτη, Ιταλία #4 Ρώμη → Βενετία → Φλωρεντία·
> οι χώρες υπάρχουν ήδη στα countries). Μην αγγίξεις i18n json, components, admin.
...
> γράψε migration, όχι reset), (2) `world-stamp-onsite-flag` (πεζοδρόμιο ή καναπές: `onSite` στο `PlaceStamp`, όπως λέει το ticket· η στήλη στο `world_stamps` είναι επέκταση, σε δεύτερο PR αν προλάβεις),
