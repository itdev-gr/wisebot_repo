...
export default function ParentDashboard({ lang }: ParentDashboardProps) {
  ...
  // Detect auth provider: Google OAuth users don't have a password
  const isGoogleUser = user?.app_metadata?.provider === 'google' ||
    (user?.app_metadata?.providers as string[] | undefined)?.includes('google');

  // Auto-unlock for fresh Google sessions (signed in within last 10 minutes)
  useEffect(() => {
    if (isGoogleUser && !isUnlocked) {
      const lastSignIn = user?.last_sign_in_at ? new Date(user.last_sign_in_at) : null;
      const isFresh = lastSignIn && (Date.now() - lastSignIn.getTime() < 10 * 60 * 1000);
      if (isFresh) setIsUnlocked(true);
    }
  }, [user, isGoogleUser, isUnlocked]);

  // Google re-auth handler
  const handleGoogleReAuth = async () => {
    ...
    try {
      ...
      setAuthError(lang === 'el' ? 'Σφάλμα επαλήθευσης' : 'Verification error');
      setVerifying(false);
    }
  };
          ...
          {isGoogleUser ? (
            ...
            <div className="space-y-4">
              <p className="text-white/40 text-xs font-bold">
                {lang === 'el'
                  ? 'Πατήστε για επαλήθευση με τον λογαριασμό Google σας'
                  : 'Click to verify with your Google account'}
              </p>
              <button
                type="button"
                onClick={handleGoogleReAuth}
                disabled={verifying}
                className="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-white font-bold text-sm flex items-center justify-center gap-3 hover:bg-white/10 hover:border-white/20 transition-all disabled:opacity-50"
              >
                ...
                {verifying
                  ? '...'
                  : (lang === 'el' ? 'Επαλήθευση με Google' : 'Verify with Google')}
              </button>
              {authError && (
            ...
            /* Email/password users: existing password gate */
