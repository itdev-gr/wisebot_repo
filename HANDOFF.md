...
`data/explore/taste/<city>.ts` (`TASTE: TasteItem[]`, τύπος στο types.ts): 6–8 πιάτα/γλυκά/
σνακ ανά πόλη, el/en, για οικογένεια: τι είναι, tip για παιδιά, ένα «ήξερες ότι», ΤΙ ΕΙΔΟΥΣ
μαγαζί (ποτέ επωνυμία), brave 1–3, `adultsOnly` (κρασί/καφές μόνο ως σημείωμα γονέων,
χωρίς κουμπί). `components/TasteCard.tsx` στην οθόνη πόλης: «Το δοκίμασα!» → `progress.tasted`
(`wb_explore_<city>`), 3 γεύσεις = σφραγίδα «Γευσιγνώστης <πόλη>» (chip στην κεφαλίδα, 🍴
στη σφραγίδα πόλης του Διαβατηρίου, μετρητής στο ευρετήριο). Registry: `loadTaste(id)` /
`hasTaste(id)` — πόλη χωρίς αρχείο = κενή λίστα, όχι crash. Test: `data/explore/taste.test.ts`
(σχήμα, δύο γλώσσες, καθόλου αλκοόλ σε παιδικό item, καμία επωνυμία). Checker για agents:
`check-taste.mjs`. Το περιεχόμενο των 17 πόλεων γράφτηκε με workflow συγγραφέας →
fact-checker → διορθωτής (Πόρτο χειρόγραφα ως πρότυπο).

## ⚡ STATUS 6 Σεπτεμβρίου 2026 — Explorer: τα σημεία βρίσκονται (feedback από το Πόρτο)

Ο ιδιοκτήτης έπαιξε το Πόρτο επιτόπου: «Σημείο 1, 2, 3» και ίδια λουκέτα παντού, δεν
ήξερε τι είναι πού. Τώρα: όνομα + emoji + αριθμός διαδρομής σε λίστα, χάρτη και κεφαλίδα
σημείου (ίδια αρίθμηση παντού)· πινακίδες ονομάτων στους markers από zoom 16 (πιο έξω
συγκρούονται — οι αριθμοί κάνουν το υπόμνημα)· «Πού είναι;» mini-map (`SpotMiniMap`) με
ακτίνα ξεκλειδώματος + κουκκίδα GPS· «Οδηγίες» Google Maps (και Apple Maps σε iOS)·
απόσταση + πυξίδα («760 μ. ↑ βόρεια», `bearingDeg`/`compass` στο utils/geo.ts)· «Πιο
κοντά σου: …». Το αίνιγμα μένει ως παιχνίδι, όχι ως εμπόδιο. Για μεγαλύτερα παιδιά:
toggle «Μυστήριο» (`wb_explore_mystery`) που κρύβει ονόματα και mini-map. Το
`unlockQuestion` γίνεται εύκολο όταν φαίνεται το όνομα — αποδεκτό, ο γονιός το ζήτησε.
...
(HR), Κοπεγχάγη (DK), Μπριζ (BE), Κρακοβία (PL). Ίδια συνταγή: συγγραφέας → ανεξάρτητος
έλεγχος → διόρθωση. Το test «η WiseBot» και το `check-city.mjs` καλύπτουν κάθε νέα πόλη.

## ⚡ STATUS 5 Σεπτεμβρίου 2026 (β) — Explorer: +10 πόλεις, 13 σύνολο

Ζήτημα ιδιοκτήτη: «top 10 προορισμοί Ευρώπης, 10 χώρες». Προστέθηκαν Ρώμη, Παρίσι,
Βαρκελώνη, Λονδίνο, Βερολίνο, Άμστερνταμ, Βιέννη, Πράγα, Βουδαπέστη, Κωνσταντινούπολη
(η τελευταία για τα ελληνόπουλα: Ιππόδρομος/Όφις των Δελφών, Αγία Σοφία, Φανάρι —
Βυζάντιο και Οθωμανοί με σεβασμό, χωρίς πικρία). Συνταγή (workflow, 3 agents ανά πόλη):
συγγραφέας → ανεξάρτητος fact-checker (γεγονότα, συντεταγμένες ±60 μ., αινίγματα,
quiz keys, kid-safety, ελληνικά) → διορθωτής, όλα με `check-city.mjs` + tsc. Τα σημεία
ανά πόλη επιλέχθηκαν για περπάτημα με παιδιά (δωρεάν/εξωτερικά, 2 clusters max).
Οι σφραγίδες χώρας δουλεύουν ήδη (1 πόλη ανά νέα χώρα ⇒ σήμα πόλης = σήμα χώρας).
Επόμενες υποψήφιες: Ντουμπρόβνικ, Κοπεγχάγη, Στοκχόλμη, Δουβλίνο, Κρακοβία, Βρυξέλλες,
Μιλάνο/Βενετία/Φλωρεντία, Θεσσαλονίκη, Ρόδος, Ηράκλειο (ελληνικές πόλεις = μεγάλο
πλεονέκτημα για τη σχολική εκδρομή).
...
**PR `claude/curriculum-enrichment` — εμπλουτισμός ύλης Μαθηματικών & Γλώσσας (ζήτημα
ιδιοκτήτη 4/9: «πάρε από το Υπουργείο Παιδείας και βάλε εμπλουτισμένη ύλη»).** Τα iep.edu.gr
/ ebooks.edu.gr είναι μπλοκαρισμένα από το proxy της session, οπότε δουλέψαμε πάνω στον
σκελετό του ΠΣ που είχε ήδη χαρτογραφηθεί 24/8 και στα κενά του: **+4 units ανά τάξη ανά
μάθημα** (Μαθηματικά + Γλώσσα, Α'–ΣΤ' = 48 units / 864 ερωτήσεις), π.χ. προβλήματα,
μοτίβα-ισότητες, δεδομένα, κλάσματα Δ', ΜΚΔ/ΕΚΠ, δυνάμεις, κύκλος, κλίμακα · δίψηφα,
συλλαβισμός, αντωνυμίες, επιρρήματα, ΥΡΑ, παθητική φωνή, ευθύς/πλάγιος λόγος,
δευτερεύουσες, ετυμολογία, επιχειρηματολογία. Πρωτότυπα όλα, αριθμητική διπλοελεγμένη
με script ανά τάξη, `audit` καθαρό. Σύνολο πλέον: **302 units** (EL 260/4.680 ερωτ.,
EN 257/4.626). Ιστορία/Φυσικά/Γεωγραφία/Αγγλικά(EFL) ΔΕΝ εμπλουτίστηκαν ακόμα — επόμενο
βήμα με την ίδια συνταγή (`scratchpad/enrich-units.json` → agents ανά τάξη → audit).
