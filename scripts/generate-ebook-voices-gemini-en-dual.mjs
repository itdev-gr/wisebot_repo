#!/usr/bin/env node
/**
 * 🎙️ WiseBot Ebook Voice Generator — Gemini TTS Dual-Key (English, Kore)
 * =======================================================================
 * Uses 2 Gemini API keys alternating to double throughput.
 *
 * Usage:
 *   GEMINI_API_KEY=key1 GEMINI_API_KEY_2=key2 node scripts/generate-ebook-voices-gemini-en-dual.mjs
 *   Options:
...
if (!API_KEY_1) { console.error('\n❌ Missing GEMINI_API_KEY!\n'); process.exit(1); }
if (!API_KEY_2) { console.error('\n❌ Missing GEMINI_API_KEY_2!\n'); process.exit(1); }
...
async function main() {
  console.log('\n🎙️  WiseBot Ebook Voice Generator — Gemini TTS DUAL KEY');
  console.log('─'.repeat(58));
  console.log(`   Model:  gemini-2.5-flash-preview-tts`);
  console.log(`   Voice:  ${VOICE}`);
  console.log(`   Keys:   2 keys alternating (double throughput)`);
  ...
  // Two genai clients alternating
  const genai1 = new GoogleGenAI({ apiKey: API_KEY_1 });
  const genai2 = new GoogleGenAI({ apiKey: API_KEY_2 });
  const clients = [genai1, genai2];
  ...
  for (const task of toRun) {
    ...
    const genai = clients[keyIndex % 2];
    const keyLabel = keyNames[keyIndex % 2];
    process.stdout.write(`   [${num}/${toRun.length}] ${label} [${keyLabel}]... `);
    ...
    try {
      ...
      keyIndex++; // alternate on success
    } catch (err) {
      failed++;
      console.log(`❌ ${err.message}`);
      if (err.message.includes('429') || err.message.includes('quota') || err.message.includes('rate')) {
        // Switch to other key immediately
        keyIndex++;
        const otherKey = keyNames[keyIndex % 2];
        const otherClient = clients[keyIndex % 2];
        console.log(`   ↩️  Rate limited on ${keyLabel} → switching to ${otherKey}...`);
        process.stdout.write(`   [${num}/${toRun.length}] ${label} [${otherKey} RETRY]... `);
        try {
          const bytes = await generateAudio(otherClient, task.text, task.outputPath);
          totalBytes += bytes;
          failed--; completed++;
          console.log(`✅ ${(bytes / 1024).toFixed(0)} KB`);
          keyIndex++;
          ...
          // If both keys are rate limited, wait 30s
          if (e2.message.includes('429') || e2.message.includes('quota')) {
            console.log('   ⏳ Both keys rate limited — waiting 30s...');
            await new Promise(r => setTimeout(r, 30000));
          }
        }
      }
    }
    if (num < toRun.length) await new Promise(r => setTimeout(r, 300));
  }
  ...
  console.log(`\n   ➡️  Next: node scripts/wav-to-mp3.mjs public/audio/ebooks/\n`);
}
